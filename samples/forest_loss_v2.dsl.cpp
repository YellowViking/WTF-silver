...
int main() {
 Matrix<Float> data = readMatrix ((String)"../samples/myData.dat"); 
Integer rows; 
rows = numRows (data); 
Integer cols; 
cols = numCols (data); 
...
for(int row = 0; row < rows; row++) {
 for(int irrelevant = 0; irrelevant < 1; irrelevant++) {
  avgScore[Index2D(row,irrelevant)] = ({Matrix<Float> pt(years,season_length);
for(int i = 0; i < years; i++) {
 for(int j = 0; j < season_length; j++) {
  pt[Index2D(i,j)] = ({Integer k; 
k = ((i * season_length) + j); 
...
for(int i = 0; i < years; i++) {
 for(int j = 0; j < years; j++) {
  comparisonMatrix[Index2D(i,j)] = ((j <= i) ? 0.0 : ({Float diff; 
diff = 0; 
Integer k; 
for(int k = 0;k <= (season_length - 1);k += 1){
...
for(int yr = 0; yr < years; yr++) {
 for(int dontcare = 0; dontcare < 1; dontcare++) {
  modelAvgScore[Index2D(yr,dontcare)] = ({Integer x; 
Integer y; 
Float score1; 
score1 = 0.0; 
for(int x = 0;x <= yr;x += 1){
for(int y = (yr + 1);y <= (years - 1);y += 1){
